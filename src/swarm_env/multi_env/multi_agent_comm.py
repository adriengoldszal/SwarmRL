...
class MASwarmTarget(gym.Env):
    ...
    def __init__(
        ...
        EzPickle.__init__(
            ...
            use_conflict_reward=use_conflict_reward,
        )
    ...
    def observe(self, agent_id):
        ...
        observation["grasper"] = (
            np.array([1])
            if len(self._agents[agent_id].grasped_entities()) > 0
            else np.array([0])
        )
    ...
    def re_init(self):
        self._map = map_dict[self.map_name](
            num_drones=self.n_agents, num_persons=self.n_targets, size=map_side_size,
        )
        self.map_size = self._map._size_area
